...
import { ModemModel, EventImage } from '../models/ModemModel'
...
const ModalEvents: React.FC<ModalEventsProps> = ({ modem, onClose, t }) => {
  const [modalStatus, setModalStatus] = useState<0 | 1 | 2>(0)
  const [gallery, setGallery] = useState<GalleryState | null>(null)
  const { fetchModemEvents } = useModemFetch()
  const { eventList } = useModem()
  ...
  const openGallery = (images: EventImage[], startIndex: number, eventTitle: string) => {
  ...
  return (
    <>
      <div className="modal-header">
        <h5 className="modal-title">Eventos del módem</h5>
              ...
              <div className="table-responsive" style={{ maxHeight: '60vh' }}>
                ...
                <table className="table mb-0">
                  <thead className="table-light sticky-top">
                    <tr>
                      <th style={{ minWidth: '300px' }} className="text-start">Evento</th>
                      <th style={{ width: '120px' }} className="text-center">Fecha</th>
                      <th style={{ width: '140px' }} className="text-center">Tipo</th>
                      <th style={{ width: '120px' }} className="text-center">Imágenes</th>
                    </tr>
                  </thead>
                  <tbody>
                          ...
                          <td>
                            ...
                            {event.modem && (
                              <small className="text-secondary d-block mt-1">
                                ...
                                {event.modem.code} - {event.modem.imei}
                              </small>
                            )}
                          </td>
                ...
                {gallery.images.map((img, idx) => (
                  <img
                    ...
                    style={{
                      ...
                      borderRadius: '6px',
                      cursor: 'pointer',
                      border: idx === gallery.currentIndex
                        ? '3px solid #0d6efd'
