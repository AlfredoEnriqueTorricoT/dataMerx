...
interface SimEventsApiResponseWrapper {
  status: number
  message: string
...
export class SimEventsApiService {
  async getEventsBySimId(
    simId: number,
    setLoading?: SetStateFn
  ): Promise<ApiResponse<EventModel[]>> {
    const res = await httpRequestWithAuth.get<SimEventsApiResponseWrapper>(
      `event/sim/${simId}`,
      setLoading
    )
