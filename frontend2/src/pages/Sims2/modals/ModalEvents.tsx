...
import { SimModel, EventImage } from '../models/SimModel'
...
const ModalEvents: React.FC<ModalEventsProps> = ({ sim, onClose, t }) => {
  const [modalStatus, setModalStatus] = useState<0 | 1 | 2>(0)
  const [gallery, setGallery] = useState<GalleryState | null>(null)
  const { fetchSimEvents } = useSimFetch()
  const { eventList } = useSim()

  useEffect(() => {
    const loadEvents = async () => {
      const result = await fetchSimEvents(sim.id)
  ...
  return (
    <>
      <div className="modal-header">
        <h5 className="modal-title">Eventos del sim</h5>
